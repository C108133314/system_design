...
| 行動者 | 使用者 | 
| 說明 | 描述使用者新增書籍資訊的過程 | 
| 完成動作 | 1. 使用者完成登入  <br> 2.點選「編輯作業」按鈕<br> 3.點選「發表評論」按鈕<br> 4.輸入書籍名稱、ISBN13碼、作者、出版社、選擇書籍分類、上傳封面和選寫內容 | 
| 替代方法 | 1. 使用者點擊「註冊」 <br> 2. 輸入使用者名稱、高科大信箱、密碼、確認密碼 <br> 3. 信箱驗證成功，完成註冊，自動登入<br> 4. 點擊「送出」完成發表  |  
| 先決條件 | 使用者必須先有帳號 | 
| 後置條件 | 發表完成，轉向使用者發表的書籍頁面 | 
